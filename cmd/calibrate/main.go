...
func main() {
	var (
		...
		perClass   = flag.String("perClass", "", "comma-separated N values to sweep per class, finds min-N per class for FP=FN=0")
	)
	flag.Parse()
	_ = perClass
	...
	// Per-class N sweep — finds minimum N per class achieving FP=FN=0 globally.
	if *perClass != "" {
		log.Printf("--- per-class N sweep ---")
		var Ns []int
		for _, s := range strings.Split(*perClass, ",") {
			var v int
			if _, err := fmt.Sscanf(s, "%d", &v); err == nil {
				Ns = append(Ns, v)
			}
		}
		// For each (class, N), count: how many entries of that fastCount
		// have their verdict correct when escalation uses exactly N.
		// We replay every entry with a "per-class" config that fixes N for
		// the test class and uses MaxNProbe for all others.
		for c := uint8(0); c <= 5; c++ {
			var classEntries int
			for _, r := range records {
				if r.fastCount == c {
					classEntries++
				}
			}
			log.Printf("== class=%d (n=%d) ==", c, classEntries)
			for _, N := range Ns {
				cfg := [6]int{search.MaxNProbe, search.MaxNProbe, search.MaxNProbe, search.MaxNProbe, search.MaxNProbe, search.MaxNProbe}
				cfg[c] = N
				fp, fn := perClassCheck(*testPath, idx, &scratch, &cfg, c)
				log.Printf("   N=%-3d  classFP=%-3d classFN=%-3d", N, fp, fn)
			}
		}
	}
...
// perClassCheck runs each entry through FraudCountTopNByClass at the per-class
// config. Only counts FP/FN attributable to the specified target class —
// i.e., entries where the fast-tier count equals targetClass.
func perClassCheck(testPath string, idx *ivf.IVFIndex, scratch *search.IVFScratch, cfg *[6]int, targetClass uint8) (int, int) {
	tf, err := os.Open(testPath)
	if err != nil {
		return -1, -1
	}
	...
	for _, e := range top.Entries {
		...
		fastCnt, _ := search.FraudCountFastOnly(&qf, &qiArr, idx, scratch, search.FastNProbe)
		if fastCnt != targetClass {
			continue
		}
		cnt := search.FraudCountTopNByClass(&qf, &qiArr, idx, scratch, cfg)
		approved := approvedFromCount(cnt)
		switch {
		case approved && !e.ExpectedApproved:
			fn++
